class HostsController < ApplicationController
  ...
  def load_hosts
    if @zabbix_url == "" || @api_zabbix == "" || @passwork_url == "" || @api_passwd == ""
      ...
      HostType.all.to_a.collect! {|type| type["id_type"]}.each do |type|
        ...
        hosts[0]["hosts"].each do |host|
          ...
          if passid == ""
            passes = JSON(send_passwork_request({"query" => new_host.hostname.split(".")[1], "tags" => ["ssh"]}, false, "/passwords/search", false, token).body)["data"]
            passes.each do |pass|
              if pass["tags"].include?("admin") && pass["name"] == new_host.hostname.split(".")[1]
                passid = pass["id"]
              end
            end
          end
